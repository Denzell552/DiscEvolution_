...
def run_model(config):
    ...
    color5=iter(plt.cm.Purples(np.linspace(0.4, 1, 8)))
    ...
    if alpha_SS > 5e-3:
        ...
        for ti in times:
            ...
            # plotting gas surface density and grain size
            ...
            # plotting dust and pebble surface density
            axes[1][0].loglog(grid.Rc, disc.Sigma_D[0], color=next(color3), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            axes[1][1].loglog(grid.Rc, disc.Sigma_D[1], color=next(color4), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))

            # plotting pressure profile
            axes[2][0].semilogx(grid.Rc, gammaP(disc), color=next(color5), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi))) # pressure gradient
            axes[2][1].loglog(grid.Rc, disc.P, color=next(color6), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi))) # Pressure
        ...
        axes[3][0].set_ylabel('Mass Flux (g/s)', fontsize=15)
        axes[3][0].set_title('Dust Mass Flux with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)
        axes[3][0].set_ylim(1e-15, 1)
        ...
        frac_data = data['gap_dust_fraction'][2:], data['gap_pebble_fraction'][2:]
        time_int = sim_params['t_interval'][2:]

        axes[4][0].semilogy(time_int, frac_data[1], color=(0.1,0.5,0.5), marker='o', label= 'Pebbles') # dust fraction across gap edge
        axes[4][0].semilogy(time_int, frac_data[0], color='0.5', marker='o', label= 'Grains') # dust fraction across gap edge
        axes[4][0].set_xlabel('Evolution Time (Myr)', fontsize=15)
        axes[4][0].set_ylabel('Fraction', fontsize=15)
        axes[4][0].set_title('Fraction of Dust and Pebble Mass Flux \n at Inner Gap Edge Compared to Outer Gap Edge Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)
