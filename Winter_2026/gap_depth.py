...
def run_model(config):
    ...
    if gap_params['on']:
        if gap_params['type'] == 'duffell2019':
            ...
            K = q**2 / (hp/Rp)**5 / disc._eos._alpha_t # Kanagawa K parameter (used for gap depth)
    ...
    # grid slice
    x2 = np.argmin(np.abs(grid.Rc/Rp - 2))
    inner_edge = np.argmin(np.abs(gap_depth - (np.max(gap_depth) + np.min(gap_depth)) / 2))
    outer_edge = np.argmin(np.abs(gap_depth[np.argmin(np.abs(grid.Rc - Rp)):] - (np.max(gap_depth) + np.min(gap_depth)) / 2)) + np.argmin(np.abs(grid.Rc - Rp))
    plt.plot(grid.Rc[:x2] / Rp, gap_depth[:x2], color='green')
    plt.axvline(grid.Rc[outer_edge] / Rp, color='red', linestyle='--', label='Gap Outer Edge')
    plt.axvline(grid.Rc[inner_edge] / Rp, color='blue', linestyle='--', label='Gap Inner Edge')
    plt.grid()
    plt.legend()
    plt.ylabel('Gap Depth (Σ/Σ₀)', fontsize=14)
    plt.xlabel('R/Rp', fontsize=14)
    plt.title(f'Gap Profile using Duffell 2019 for 1 Mj with hp/Rp={disc.H[np.argmin(np.abs(grid.Rc - Rp))]/Rp:.2f}', fontsize=16)
    ...
    # saving figure
    #plt.savefig(f"Winter_2026/Figs/Gaussian**4_gap_1Mj.png")

    data = {}
    data["R/Rp"] = list(grid.Rc / Rp)
    data['depth'] = list(gap_depth)

        # Save data to json
    #with open(f"Winter_2026/Data/Gaussian**4_gap_1Mj.json", "w") as f:
    #    json.dump(data, f)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "simulation": {
            "t_initial": 0,
            "t_final": 1,
            "t_interval": [0, 0.01, 0.05, 0.1, 0.25], #Myr
        },
        "disc": {
            "alpha": 1e-3,
            "M": 0.05, # solar masses
        ...
        "planets": {
            ...
            "Rp": [4.5], #[1, 5, 10, 20, 30], # initial position of embryo [AU]
