...
def run_model(config):
    ...
    color5=iter(plt.cm.Purples(np.linspace(0.4, 1, 9)))
    color6=iter(plt.cm.Oranges(np.linspace(0.4, 1, 9)))
    color7=iter(plt.cm.BuGn(np.linspace(0.4, 1, 9)))
    color8=iter(plt.cm.PuBu(np.linspace(0.4, 1, 9)))
    ...
    if alpha_SS > 5e-3:
        ...
        for ti in times:
            ...
            axes[0][1].loglog(grid.Rc, disc.grain_size[1], color=next(color2), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))

            # calculate drift timescale
            t_drift_dust = 2 * np.pi * grid.Rc / np.abs(disc.v_drift[0])  # in years
            t_drift_pebbles = 2 * np.pi * grid.Rc / np.abs(disc.v_drift[1])  # in years

            axes[1][0].loglog(grid.Rc, t_drift_dust, color=next(color3), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            axes[1][1].loglog(grid.Rc, t_drift_pebbles, color=next(color4), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))

            # plotting stokes number
            axes[2][0].loglog(grid.Rc, disc.Stokes()[0], color=next(color5), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            axes[2][1].loglog(grid.Rc, disc.Stokes()[1], color=next(color6), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            ...
            axes[3][0].loglog(grid.Rc, t_drag_dust, color=next(color7), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            axes[3][1].loglog(grid.Rc, t_drag_pebbles, color=next(color8), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
        ...
        axes[1][0].set_ylabel('Drift Timescale (yr)', fontsize=15)
        axes[1][0].set_title('Dust Drift Timescale with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)

        axes[1][1].minorticks_off()
        axes[1][1].legend(fontsize=10)
        axes[1][1].set_xlabel('Radius (AU)', fontsize=15)
        axes[1][1].set_ylabel('Drift Timescale (yr)', fontsize=15)
        axes[1][1].set_title('Pebble Drift Timescale with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)

        axes[2][0].minorticks_off()
        axes[2][0].legend(fontsize=10)
        axes[2][0].set_xlabel('Radius (AU)', fontsize=15)
        axes[2][0].set_ylabel('Stokes Number', fontsize=15)
        axes[2][0].set_title('Dust Stokes Number with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)

        axes[2][1].minorticks_off()
        axes[2][1].legend(fontsize=10)
        axes[2][1].set_xlabel('Radius (AU)', fontsize=15)
        axes[2][1].set_ylabel('Stokes Number', fontsize=15)
        axes[2][1].set_title('Pebble Stokes Number with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17)

        axes[3][0].minorticks_off()
        axes[3][0].legend(fontsize=10)
        axes[3][0].set_xlabel('Radius (AU)', fontsize=15)
        axes[3][0].set_ylabel('Drag Timescale (yr)', fontsize=15)
        axes[3][0].set_title('Dust drag timescale with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17) 

        axes[3][1].minorticks_off()
        axes[3][1].legend(fontsize=10)
        axes[3][1].set_xlabel('Radius (AU)', fontsize=15)
        axes[3][1].set_ylabel('Drag Timescale (yr)', fontsize=15)
        axes[3][1].set_title('Pebble drag timescale with Ψ = {}'.format(wind_params["psi_DW"]), fontsize=17) 
