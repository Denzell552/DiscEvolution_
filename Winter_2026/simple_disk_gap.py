...
def run_model(config):
    ...
    color9=iter(plt.cm.Oranges(np.linspace(0.4, 1, 8)))
    color10=iter(plt.cm.Reds(np.linspace(0.4, 1, 8)))
    ...
    if alpha_SS > 5e-3:
        ...
        for ti in times:
            ...
            axes[1][0].loglog(grid.Rc, disc.Sigma_D[0], color=next(color9), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
            axes[1][1].loglog(grid.Rc, disc.Sigma_D[1], color=next(color10), label='{:.3f} Myr'.format(t / (1.e6 * 2 * np.pi)))
        ...
        # saving figure
        fig.savefig(f"Winter_2026/Figs/gap_test_winds_mig_psi={wind_params['psi_DW']}_alpha={disc_params['alpha']:.1e}_M={disc_params['M']:.1e}_Rd={disc_params['Rd']:.1e}.png")

        # Save data to json
        with open(f"Winter_2026/Data/gap_test_winds_mig_psi={wind_params['psi_DW']}_alpha={disc_params['alpha']:.1e}_M={disc_params['M']:.1e}_Rd={disc_params['Rd']:.1e}.json", "w") as f:
            json.dump(data, f)
